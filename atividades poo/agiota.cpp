...
public:
    ...
    int getBalance() {
      ...
      // 1 modo que da p fazer
        ...
        return balance;
        /*o modo que eu achei legal de fazer:
           int total = std::accumulate(operations.begin(), operations.end(), 0,
        [](int acc, const std::shared_ptr<Operation>& operation) {
            if (operation->getLabel() == "GIVE" || operation->getLabel() == "PLUS") {
                return acc + operation->getValue();
            } else if (operation->getLabel() == "TAKE") {
                return acc - operation->getValue();
            }
            return acc;
        }
