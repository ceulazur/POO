...
/*
o que fez: meu amigo, eu já nem sei mais, cliente, agiota
espero ter colocado todo o código direit aqui agora, mas como estou cansada n vou conferir agr, irei confirmar depois de descansar
com quem fez: com o amigo de todos, o famoso chat gpt (que mais me enlouqueceu q outra coisa)
o que aprendeu: por hora acho que nada, to nem conseguindo mais pensar no momento, vou pegar para ver
se termino o codigo mais tarde, ou melhor, mais cedo, estando menos cansada e sonolenta para poder te dizer ao certo
no q q deu o aprendizado.
mas na real, eu aprendi sobre uma biblioteca nova chamada numeric e algo como usar o acumulate
então para ser bem sincera eu estou indo para algum lugar no processo de aprendizagem
quanto tempo levou: as 4 horas "obrigatórias" e faltou
*/
//atualizações
/*
o que fiz: tudo
com quem fiz: com o seu vídeo
o que aprendi: que meu deus mil coisas ligam umas as outras e se não tiver organização vira uma zona, que desesperador
e no fim aprendi melhor isso, ligar uma coisa na outra, mais funcionalidades do c++, o uso de ponteiro e filter
quanto tempo levei: umas duas horas
*/
...
public:
    ...
    int getBalance() {
        ...
        for (auto operation : operations) {
            if (operation->getLabel() == Label::GIVE || operation->getLabel() == Label::PLUS) {
                balance += operation->getValue();
            } else if (operation->getLabel() == Label::TAKE) {
                balance -= operation->getValue();
            }
        }
        return balance;  
